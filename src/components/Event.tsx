...
const Event: FC<IEventProps> = (props: IEventProps) => {
  const getTime = () => {
    // console.log(
    //   "🚀 ~ file: Event.tsx ~ line 38 ~ getTime ~ props.event._timestamp",
    //   ethers.BigNumber.from(props.event._timestamp).toNumber()
    // );
    const date = new Date(props.event._timestamp as unknown as number);
